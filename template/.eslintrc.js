module.exports = {
  ...
  globals: {
    // 项目的全局变量
    // example1: true, // 允许被重新赋值
    // example2: false // 不允许被重新赋值
    _:true
  },
  rules: {
    /* 以下是通用的规则 */
    ...
    // 禁止将 async 函数作为 new Promise 的回调函数
    ...
    // @off foo == null 比较常用，可以用于判断 foo 不是 undefined 并且不是 null，故允许此写法
    'no-eq-null': 'off',
    // 禁止使用 eval
    'no-eval': 'error',
    // 禁止修改原生对象
    ...
    // @off 不作限制
    'id-match': 'off',
    // @fixable 箭头函数的函数体必须与箭头在同一行，或者被括号包裹
    ...
    // @fixable 类的成员之间是否需要空行
    // @off 有时为了紧凑需要挨在一起，有时为了可读性需要空一行
    ...
    // @fixable 限制组件的命名格式为 PascalCase
    // 'ignores' 中包含 Vue.js、iView 和 Electron 中的一些特殊标签
    ...
    // @fixable 内部没有内容时，组件必须自闭合
    // @off 不作限制
    'vue/html-self-closing': 'off',
    // 禁止使用 v-html 以防 XSS 攻击
    // @off 有时需要使用
    'vue/no-v-html': 'off',
    ...
    // 要求组件的 props 有 default 值
    // @off 为每个 prop 设置 default 值过于繁琐
